<?php
/**
 * csportfolio functions and definitions
 *
 ...
 */
function csportfolio_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on csportfolio, use a find and replace
		* to change 'csportfolio' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'csportfolio', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'csportfolio' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'csportfolio_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'csportfolio_setup' );
 ...
 */
function csportfolio_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'csportfolio_content_width', 640 );
}
add_action( 'after_setup_theme', 'csportfolio_content_width', 0 );
 ...
 */
function csportfolio_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'csportfolio' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'csportfolio' ),
 ...
 */
function csportfolio_scripts() {
	wp_enqueue_style( 'csportfolio-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'csportfolio-style', 'rtl', 'replace' );

	wp_enqueue_script( 'csportfolio-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
add_action('wp_enqueue_scripts', 'mousemove_script');
